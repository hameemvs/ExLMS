...
export function Login() {
  return (
    <div className="flex justify-center items-center min-h-screen bg-gradient-to-br from-slate-100 via-indigo-50/30 to-slate-100 p-4">
      <div className="rounded-[2rem] shadow-2xl shadow-indigo-200/30 flex w-full max-w-[850px] overflow-hidden relative min-h-[550px] bg-white animate-scale-in">
        
        <div className="w-[45%] bg-gradient-to-b from-[#1E1B4B] via-[#312E81] to-[#4C1D95] p-10 flex flex-col justify-center items-center text-white relative z-10">
          <CloudSeparator />
          
          <div className="absolute top-[-40px] right-[-40px] w-[140px] h-[140px] bg-indigo-400/15 rounded-full blur-2xl pointer-events-none"></div>
          <div className="absolute bottom-[-40px] left-[-30px] w-[120px] h-[120px] bg-violet-400/15 rounded-full blur-2xl pointer-events-none"></div>
          
          <div className="mb-6 relative z-40">
            <h2 className="text-xl font-medium tracking-wide text-indigo-100">Welcome to</h2>
          </div>
          
          <div className="bg-white rounded-full p-5 mb-4 shadow-lg shadow-indigo-900/30 relative z-40">
          ...
          <div className="text-[11px] text-center opacity-70 mt-auto px-4 leading-relaxed font-light relative z-40">
        ...
        <div className="w-[55%] bg-white p-12 flex flex-col justify-center pl-16 relative z-30">
          <h2 className="text-xl font-bold text-slate-800 mb-10 text-center mr-8">Sign in to your account</h2>
          
          <form className="space-y-6 w-full max-w-sm">
            <div className="relative">
              <label className="text-xs font-semibold text-slate-600 block mb-2">E-mail Address</label>
              <input 
                type="email" 
                placeholder="Enter your email" 
                className="w-full border-b-[1.5px] border-indigo-200 pb-2 focus:outline-none focus:border-indigo-500 text-sm text-slate-800 bg-transparent placeholder-slate-300 transition-colors"
              />
              <Check size={16} className="absolute right-0 bottom-3 text-indigo-300" />
            </div>
            ...
            <div className="flex items-center gap-2 pt-2">
              <input type="checkbox" className="w-3.5 h-3.5 accent-indigo-600 rounded cursor-pointer" id="remember" />
              <label htmlFor="remember" className="text-[10px] text-slate-500 cursor-pointer">
                By Signing In I Agree with <span className="text-indigo-600 font-medium">Terms & Conditions</span>
              </label>
            </div>

            <div className="flex gap-4 pt-6">
               <Link to="/register" className="flex-1">
                 <button type="button" className="w-full py-2.5 btn-gradient rounded-full text-sm font-semibold">
                   Sign Up
                 </button>
               </Link>
               <Link to="/dashboard" className="flex-1">
                 <button type="button" className="w-full py-2.5 bg-white text-slate-500 border border-slate-200 rounded-full text-sm font-semibold hover:bg-slate-50 hover:border-slate-300 transition-all duration-200">
                   Sign In
                 </button>
               </Link>
            </div>
